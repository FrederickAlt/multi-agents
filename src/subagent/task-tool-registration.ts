...
export function configureTaskToolForRuntime(
	...
	if (canSpawn) {
		...
		targetPi.registerTool({
			...
			promptGuidelines: [
				...
				"Task and wait_for_agent results report child context usage when available; before using resume, consider whether the agent has enough remaining context to finish the task.",
	...
	targetPi.registerTool({
		...
		promptGuidelines: [
			...
			"Review reported child context usage before resuming an agent; decide whether its remaining context is sufficient to finish the task.",
