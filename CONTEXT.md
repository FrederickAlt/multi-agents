...
- **`wait_for_agent`**: A tool that retrieves output from one or more sub-agents by ID. Blocks the parent turn until any listed agent finishes or a timeout expires. Returns structured per-agent output. Works on both async agents and finished blocking agents. Accepts `timeout` (minutes, default 5) and `kill_on_timeout` (boolean, default false).
...
- **`kill_on_timeout`**: Parameter on `wait_for_agent` that escalates timeout handling. When the timeout fires with `kill_on_timeout: true`, the sub-agent receives a soft-kill warning to finish within the same duration. If it still does not finish, the session is hard-aborted (transcript persists for resume).
- **Soft-kill**: Informing a running sub-agent that it should finish within a deadline, giving it a chance to produce a final answer before being forcibly terminated.
- **Hard-abort**: Forcibly terminating a sub-agent session that failed to meet a kill deadline. The session transcript persists on disk and the agent can be resumed.
- **Unconsumed agent**: An async agent that has finished execution but whose output has not yet been retrieved via `wait_for_agent`. Tracked by the notification system and re-reminded every ~5 turns until consumed.
- **Notification system**: Turn-boundary injection of consolidated `[System]` messages listing finished-but-unconsumed async agents. A new agent completion triggers the message on the next turn. If unconsumed agents remain after 5 turns with no new completions, the message is re-injected. When the user is mid-typing, the notification batches with the user's message.
